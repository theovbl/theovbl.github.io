...
<body>
<p>Votre component est ajouté !</p>
<?php
        if (isset($_POST) && !empty($_POST)) { //Il vérifie si une requête POST a été effectuée et si les données POST ne sont pas vides
            $select = $bdd->prepare('SELECT * FROM component WHERE id=?');
            $select->execute(array( $_POST['id']));
            $select = $select->fetchAll();
            if (count($select) <= 0) {
                $insert = $bdd->prepare('INSERT INTO component(id, name, contact, description) VALUES (?, ?, ?, ?)');
