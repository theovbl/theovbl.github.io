...
<?php 
    ...
    <?php 
        if (isset($_POST) && !empty($_POST)) { // On a créer une base de données users et on a rangé 
            $insert = $bdd->prepare('INSERT INTO users(username, email, password) VALUES (?, ?, ?)');
            $insert->execute(array (
                $_POST['username'],
                $_POST['email'],
                password_hash($_POST['password'], PASSWORD_ARGON2ID) // c'est très renforcé argon2id 
            ));
