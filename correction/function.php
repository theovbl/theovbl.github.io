...
<body>
    <?php
        ...
        // RemplacerLesLettres c'est le nom de la fonction
        //$phrase c'est le paramètre, on pourrait mettre un deuxième paramètre
        // Grace au paramètre on va pouvoir réutiliser plusieurs fois la fonction
        ...
        function DernierElementTableau($tab) {
            if (!empty($tab)) {
                ...
                // count c'est la longueur du tableau ( le nombre d'élément )
            } else {
                return null;
                # La condition return est une entre gillemet une fonction de PHP qui envoie l'élément
                # Que on lui donne à l'endroit ou on à appelé la fonction 
            }
        }

        $tab = [10, 10, 47.6579, 'cléopatre', 'autruche' ];
        ...
        function PremierElementTableau($tab) {
            if (!empty($tab)) {
                return $tab[0];
                // count c'est la longueur du tableau on l'utilise unquement pour des tableaux
                # return $tab[count($tab) - count($tab)]; autre solution // la longueur du tableau - la longueur du tableau
            }
            return null;
