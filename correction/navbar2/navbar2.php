...
<head>
    ...
    <title>Name</title>
    <link rel="stylesheet" href="../navbar2/navabar2.css">
</head>
<body>
    <nav>
        <div>        
    ...
    <script> /* On utilise Jquery : jquery c'est du Js simplifié */
        $('#menu').click(function() {
            if ($('nav ul').is(':visible')) {
                $('nav ul').animate({
                    opacity: 0,
                }, 500, function() {
                    $('nav ul').hide() /* Tu caches */
                })
            } else {
                $('nav ul').show() /* show c'est montrer */
                $('nav ul').animate({ /* animer */
                    opacity: 1,
                    left: '50px'
                }, 500)
